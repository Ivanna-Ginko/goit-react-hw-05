...
//const url = 'https://api.themoviedb.org/3/search/movie?query={searchValues}include_adult=false&language=en-US&page=1';
...
export const fetchResults = async values => {
    const response = await axios.get(`https://api.themoviedb.org/3/search/movie?query=${values}include_adult=false&language=en-US&page=1`, options)
    return response;
}
